...
class siren_decompo_1(nn.Module):
    def __init__(self, in_features:int, order:int,device="auto"):
        self.out_size = 64
        super().__init__()
        self.siren = Siren(in_features=in_features, out_features=self.out_size, hidden_features=self.out_size, hidden_layers=0, outermost_linear=True).to(device)
...
class angular_2(nn.Linear):
    ...
    def forward(self, x:torch.Tensor)->torch.Tensor:
        ...
        return torch.cat([cos*torch.abs(cos),sin*torch.abs(sin),x],dim=1)
